...
object DrawableUtils {
    ...
    /**
     * 处理图片，耗时方法，带一下 suspend
     */
    suspend fun execute(resPath: String): String {
        ...
        getDensityQualifierList().sortedByDescending { it.priority }.forEach { densityQualifier ->
            ...
            tempBaseDpiFolderNameList.forEach { tempBaseDpiFolderName ->
                ...
                if (count > 1) { // 数量大于 1 才需要真正选为基准文件夹，因为只有一项的时候就不需要进行多余资源处理了
        ...
        val result = "所有文件都处理完毕，处理前文件 $fileNumBeforeDelete 个，处理后文件 $fileNumAfterDelete 个，共删除文件 $deleteNum 个"
        println(" ========================= $result ========================= ")
        return result
    }
